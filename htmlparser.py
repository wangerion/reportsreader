...
# Opening all the reports from a directory
for report in os.listdir("reports"):
    with open(os.path.join("reports", report)) as fp:
    ...
    for row in tableuptime:
        ...
        if len(uptime) == 2:
            ...
            if uptime[0].isdigit() and int(uptime[0]) >= 4:
                ...
                print(f"In report {report} Server: {cells[0].text} has the following uptime: {cells[1].text}")
        else:
            print(f"In report {report} Server: {cells[0].text} has the following uptime: {cells[1].text}")
    ...
    for row in tableusage:
        ...
        for data in list(split(cells, 5)):
            ...
            if data[4].text.strip('%').isdigit() and float(data[4].text.strip('%')) >= 85:
                print(f"In report {report} Mountpoint: {data[0].text} is at: {data[4].text}")
